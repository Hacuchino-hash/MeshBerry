...
{
#ifdef USE_MESHCORE
    // MeshCore handles routing - skip Meshtastic mesh processing
    LOG_DEBUG("MeshService::handleToRadio skipped - MeshCore mode");
    return;
#endif
...
{
#ifdef USE_MESHCORE
    // MeshCore handles routing - router is NULL, just release packet
    packetPool.release(p);
    return;
#endif
    // Runtime NULL check as defense-in-depth
    if (!router) {
        LOG_WARN("Router NULL in sendToMesh, releasing packet");
        packetPool.release(p);
        return;
    }
